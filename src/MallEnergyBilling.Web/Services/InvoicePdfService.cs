...
public sealed class InvoicePdfService
{
    ...
    private static Document BuildDocument(Invoice i)
    {
        ...
        var head=s.AddTable();head.AddColumn(Unit.FromMillimeter(105));head.AddColumn(Unit.FromMillimeter(55));var hr=head.AddRow();var brand=hr.Cells[0].AddParagraph();var mark=brand.AddFormattedText("WATCH DOG EM",TextFormat.Bold);mark.Font.Size=19;mark.Font.Color=teal;brand.AddLineBreak();var sub=brand.AddFormattedText("UTILITY METERING & BILLING",TextFormat.Bold);sub.Font.Size=8;sub.Font.Color=gray;var inv=hr.Cells[1];inv.Shading.Color=utility==UtilityType.Water?Color.Parse("#075985"):navy;inv.Format.LeftIndent=Unit.FromMillimeter(5);inv.VerticalAlignment=VerticalAlignment.Center;var ip=inv.AddParagraph(invoiceKind);ip.Format.Font.Bold=true;ip.Format.Font.Size=16;ip.Format.Font.Color=Colors.White;var ino=inv.AddParagraph(i.InvoiceNumber);ino.Format.Font.Color=Colors.White;ino.Format.SpaceBefore=Unit.FromMillimeter(2);
