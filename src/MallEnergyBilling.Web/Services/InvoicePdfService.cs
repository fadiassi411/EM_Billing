...
public sealed class InvoicePdfService
{
    ...
    private static Document BuildDocument(Invoice i)
    {
        ...
        var reading=s.AddTable();reading.Borders.Color=Color.Parse("#D9E3E6");reading.Borders.Width=.5;reading.AddColumn(Unit.FromMillimeter(40));reading.AddColumn(Unit.FromMillimeter(40));reading.AddColumn(Unit.FromMillimeter(40));reading.AddColumn(Unit.FromMillimeter(40));var rh=reading.AddRow();rh.Shading.Color=pale;Header(rh,"METER / SOURCE","PREVIOUS READING","CURRENT READING","CONSUMPTION");var rv=reading.AddRow();Cell(rv.Cells[0],$"{i.Meter?.Name}\n{i.Meter?.SerialNumber}\n{i.Meter?.PowerSource}",true);Cell(rv.Cells[1],$"{i.OpeningReading:N2} kWh");Cell(rv.Cells[2],$"{i.ClosingReading:N2} kWh");Cell(rv.Cells[3],$"{i.ConsumptionKwh:N2} kWh",true);
