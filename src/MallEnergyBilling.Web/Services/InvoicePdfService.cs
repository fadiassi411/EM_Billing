...
public sealed class InvoicePdfService
{
    ...
    private static Document BuildDocument(Invoice i)
    {
        ...
        var d = new Document(); d.Info.Title = $"Black Dog Energy Invoice {i.InvoiceNumber}"; d.Info.Author = "Black Dog";
        ...
        var footer = s.Footers.Primary.AddTable(); footer.AddColumn(Unit.FromMillimeter(115)); footer.AddColumn(Unit.FromMillimeter(45)); var fr=footer.AddRow();fr.Cells[0].AddParagraph("Black Dog  |  Watch Every Watt").Format.Font.Color=gray;var fp=fr.Cells[1].AddParagraph();fp.Format.Alignment=ParagraphAlignment.Right;fp.AddText("Page ");fp.AddPageField();fp.AddText(" of ");fp.AddNumPagesField();

        var head=s.AddTable();head.AddColumn(Unit.FromMillimeter(105));head.AddColumn(Unit.FromMillimeter(55));var hr=head.AddRow();var brand=hr.Cells[0].AddParagraph();var mark=brand.AddFormattedText("BLACK DOG",TextFormat.Bold);mark.Font.Size=19;mark.Font.Color=teal;brand.AddLineBreak();var sub=brand.AddFormattedText("WATCH EVERY WATT",TextFormat.Bold);sub.Font.Size=8;sub.Font.Color=gray;var inv=hr.Cells[1];inv.Shading.Color=navy;inv.Format.LeftIndent=Unit.FromMillimeter(5);inv.VerticalAlignment=VerticalAlignment.Center;var ip=inv.AddParagraph("ENERGY INVOICE");ip.Format.Font.Bold=true;ip.Format.Font.Size=16;ip.Format.Font.Color=Colors.White;var ino=inv.AddParagraph(i.InvoiceNumber);ino.Format.Font.Color=Colors.White;ino.Format.SpaceBefore=Unit.FromMillimeter(2);
