...
public sealed class InvoicePdfService
{
    ...
    {
        var utility=i.Meter?.UtilityType??UtilityType.Electricity;var unit=i.Meter?.Unit??"kWh";var invoiceKind=utility==UtilityType.Water?"WATER INVOICE":"ENERGY INVOICE";
        var teal = Color.Parse("#0C8C84"); var navy = Color.Parse("#0C2637"); var pale = Color.Parse("#EAF5F4"); var gray = Color.Parse("#667781");
        var d = new Document(); d.Info.Title = $"Watchdog Energy Management Invoice {i.InvoiceNumber}"; d.Info.Author = "Watchdog Energy Management";
        ...
        var footer = s.Footers.Primary.AddTable(); footer.AddColumn(Unit.FromMillimeter(115)); footer.AddColumn(Unit.FromMillimeter(45)); var fr=footer.AddRow();fr.Cells[0].AddParagraph("Watchdog Energy Management  |  Watch Every Watt").Format.Font.Color=gray;var fp=fr.Cells[1].AddParagraph();fp.Format.Alignment=ParagraphAlignment.Right;fp.AddText("Page ");fp.AddPageField();fp.AddText(" of ");fp.AddNumPagesField();

        var head=s.AddTable();head.AddColumn(Unit.FromMillimeter(105));head.AddColumn(Unit.FromMillimeter(55));var hr=head.AddRow();var brand=hr.Cells[0].AddParagraph();var mark=brand.AddFormattedText("WATCHDOG ENERGY MANAGEMENT",TextFormat.Bold);mark.Font.Size=17;mark.Font.Color=teal;brand.AddLineBreak();var sub=brand.AddFormattedText("UTILITY METERING & BILLING",TextFormat.Bold);sub.Font.Size=8;sub.Font.Color=gray;var inv=hr.Cells[1];inv.Shading.Color=utility==UtilityType.Water?Color.Parse("#075985"):navy;inv.Format.LeftIndent=Unit.FromMillimeter(5);inv.VerticalAlignment=VerticalAlignment.Center;var ip=inv.AddParagraph(invoiceKind);ip.Format.Font.Bold=true;ip.Format.Font.Size=16;ip.Format.Font.Color=Colors.White;var ino=inv.AddParagraph(i.InvoiceNumber);ino.Format.Font.Color=Colors.White;ino.Format.SpaceBefore=Unit.FromMillimeter(2);
        ...
        var reading=s.AddTable();reading.Borders.Color=Color.Parse("#D9E3E6");reading.Borders.Width=.5;reading.AddColumn(Unit.FromMillimeter(40));reading.AddColumn(Unit.FromMillimeter(40));reading.AddColumn(Unit.FromMillimeter(40));reading.AddColumn(Unit.FromMillimeter(40));var rh=reading.AddRow();rh.Shading.Color=pale;Header(rh,"METER / UTILITY","PREVIOUS READING","CURRENT READING","CONSUMPTION");var rv=reading.AddRow();Cell(rv.Cells[0],$"{i.Meter?.Name}\n{i.Meter?.SerialNumber}\n{utility}",true);Cell(rv.Cells[1],$"{i.OpeningReading:N2} {unit}");Cell(rv.Cells[2],$"{i.ClosingReading:N2} {unit}");Cell(rv.Cells[3],$"{i.ConsumptionKwh:N2} {unit}",true);
        s.AddParagraph().Format.SpaceAfter=Unit.FromMillimeter(5);

        var bill=s.AddTable();bill.Borders.Color=Color.Parse("#D9E3E6");bill.Borders.Width=.5;bill.AddColumn(Unit.FromMillimeter(85));bill.AddColumn(Unit.FromMillimeter(35));bill.AddColumn(Unit.FromMillimeter(40));var bh=bill.AddRow();bh.Shading.Color=navy;HeaderWhite(bh,"DESCRIPTION","RATE / QTY","AMOUNT");AddLine(bill,$"{utility} consumption",$"{i.TariffPerKwh:N4} / {unit}",i.EnergyCharge,i.Currency);AddLine(bill,"Fixed and service charges","",i.FixedCharges+i.OtherCharges,i.Currency);AddLine(bill,"Tax / VAT","",i.Tax,i.Currency);AddLine(bill,"Discount","",-i.Discount,i.Currency);AddLine(bill,"Previous balance","",i.PreviousBalance,i.Currency);
        s.AddParagraph().Format.SpaceAfter=Unit.FromMillimeter(5);
