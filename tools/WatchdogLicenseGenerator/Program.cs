...
static int IssueLicense(string[] args)
{
    ...
    if (!int.TryParse(Required(values, "meters"), out var meters) || meters < 50 || meters % 50 != 0)
    ...
    var payload = new LicensePayload(
        ...
        expiry);
    var document = LicenseDocument.Create(payload, File.ReadAllText(privatePath));
    Directory.CreateDirectory(Path.GetDirectoryName(Path.GetFullPath(output))!);
    File.WriteAllText(output, document.ToJson());
...
{
    var values = Parse(args);
    var document = LicenseDocument.Parse(File.ReadAllText(Required(values, "license")));
...
static int ShowHelp()
{
    Console.WriteLine("Watchdog offline license generator");
