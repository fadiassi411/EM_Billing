...
public sealed class MainForm : Form
{
    ...
    private void BuildUi()
    {
        ...
        header.Controls.Add(new Label { Text = "Secure offline licensing for Watchdog Energy Management", ForeColor = Color.FromArgb(183, 222, 229), AutoSize = true, Location = new Point(31, 62) });
        Controls.Add(header);
        ...
        AddField(table, "Customer name", customer);
        AddField(table, "Installation ID", installation);
        AddField(table, "Meter capacity", meters);
    ...
    private void Generate(object? sender, EventArgs e)
    {
        try
        {
            ...
            if (meters.Value % 50 != 0) throw new InvalidOperationException("Meter capacity must be a block of 50.");
            ...
            var document = LicenseDocument.Create(payload, File.ReadAllText(keyPath));
    ...
    private void Verify(object? sender, EventArgs e)
    {
        ...
        try
        {
            ...
            var document = LicenseDocument.Parse(File.ReadAllText(dialog.FileName));
